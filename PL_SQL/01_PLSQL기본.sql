...
- 일반적인 프로그래밍과는 차이가 있지만, 오라클 내부에서 적절한 처리를 위해
 적용해 줄 수 있는 절차지향적(순서: 위 -> 아래) 코드 작성 방식입니다.
- 쿼리문의 집합으로 어떠한 동작을 일괄 처리하기 위한 용도로 사용합니다.
*/

-- 익명 블록

SET SERVEROUTPUT ON; -- 출력문 활성화를 먼저 ON시켜줘야 한다.

DECLARE -- 변수를 선언하는 구간 (선언부)
    
    emp_num NUMBER; -- 변수+타입 선언
    
BEGIN -- 코드를 실행하는 시작 구간 (실행부)

    emp_num := 10; -- oracle의 대입연산자(:=)
    DBMS_OUTPUT.put_line(emp_num); -- 자바의 sop 같은 것.
...
BEGIN
    SELECT
        ...
        d.department_name
    INTO
        v_emp_name, v_dep_name -- 조회 결과를 변수에 대입 가능
    FROM employees e
...
END; -- 결과: Steven-Executive
-- QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ. 근데 system계정
