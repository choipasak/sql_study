...
SELECT
    ...
    d.department_name
FROM employees e LEFT JOIN departments d
ON e.department_id = d.department_id -- e.employee_id로 안하려면 LEFT JOIN을 해줘야함: INNER JOIN을 하면 조건에 해당되지 않으면 출력조차 해주지 않기 때문
WHERE e.first_name || ' ' || e.last_name = 'Steven King';
-- PK 값이 컬럼을 찾아서 JOIN해주기!
...
SELECT
    *
FROM employees e CROSS JOIN departments d; -- 2889개의 행 출력됨
...
FROM employees e JOIN jobs j
ON e.job_id = j.job_id
WHERE j.job_title IN('Stock Manager','Stock Clerk'); -- IN을 써도 된다.
/*
...
ON e.department_id = d.department_id
WHERE e.employee_id IS NULL; -- 해당 부서에 직원이 없다는 것은 사원 번호가 null인 것.(기준: departments)
